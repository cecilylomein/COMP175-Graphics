...
string filename;
std::unordered_map<std::string, SceneColor*> imagedata;
...
nearestObj findNearestIntersection(Point eye, Vector d);
SceneColor findColor (nearestObj obj, Point eye, Vector d);
...
SceneColor raytrace(int w, int h) {
    ...
    d.normalize();
    return findColor(findNearestIntersection(eye_world, d), eye_world, d);
}
...
nearestObj findNearestIntersection(Point eye, Vector d){
    ...
    for (int i = 0; i < tree.size(); i++) {
        for (ScenePrimitive *prim : tree[i]->primitives) {
            float t;
...
SceneColor findColor (nearestObj obj, Point eye, Vector d) {
    ...
    Matrix m_inv = obj.m_inv;

    Point intersection = eye + d * (t_min - .0001);
...
inline static SceneColor bound(SceneColor c) {
    if (c.r > 1.0) c.r = 1.0;
    if (c.g > 1.0) c.g = 1.0;
    if (c.b > 1.0) c.b = 1.0;
    return c;
...
SceneColor computeSceneColor(SceneMaterial material, Vector Nhat, Point pos) {
    ...
    Look.normalize();
    Vector Vhat = pos - cameraData.pos;
    Vhat.normalize();

    for (int m = 0; m < nLights; m++) {
        SceneLightData lData;
        parser->getLightData(m, lData);
        SceneColor lm = bound(lData.color);
        Vector Lhatm = lData.pos - pos;
        Vector Lvec  = pos - lData.pos;
        Lhatm.normalize();
        Vector Rhatm = Lvec - 2 * (dot (Lvec, Nhat)) * Nhat;
        Rhatm.normalize();

        if (findNearestIntersection(pos, Lhatm).t < 0) { 
            if (dot(Nhat, Lhatm) > 0) {
            ...
            if (dot(Rhatm, Vhat) > 0) {
                color.r += (ks * Os.r * pow(dot(Rhatm, Vhat), f)); 
                color.g += (ks * Os.g * pow(dot(Rhatm, Vhat), f)); 
                color.b += (ks * Os.b * pow(dot(Rhatm, Vhat), f)); 
            }
        }
    }
    ...
    if (recursion_remaining > 0) {
        recursion_remaining--;
        Vector d = Look - 2 * (dot (Look, Nhat)) * Nhat;
        d.normalize();
...
std::vector<SceneColor*> parsePixels(std::string filename) {
...
*/
    ...
    std::vector<SceneColor*> pixels = new vector<SceneColor*>;
    SceneColor c;
...
void texturemap_primitives(std::vector<ScenePrimitive*> primitives){
    
    SceneFileMap* texture;
    for (int i = 0; i < primitives.size(); i++) {
        texture = primitives[i].textureMap;
        if (texture->isUsed) {
            imagedata[filename] = new vector<SceneColor>;
            imagedata[filename] = parsePixels(filename);
        }
    }
...
void flatten_tree(SceneNode* root, SceneTransformation* parent_transforms) {
	...
	tree.push_back(node);
    texturemap_primitives(root->primitives);
